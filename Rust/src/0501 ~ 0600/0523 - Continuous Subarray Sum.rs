...
//      即 (x - y) % k = 0 。
//
//      所以我们可以维护每个前缀和模 k 的余数 remain 第一次出现的下标 j 。
//
...
impl Solution {
    pub fn check_subarray_sum(nums: Vec<i32>, k: i32) -> bool {
        // remain_to_first_index 维护每一个前缀和 % k 第一次出现的下标
        let mut remain_to_first_index = HashMap::new();
        // 初始前缀和 0 % k 的下标为 -1 ，方便处理整个前缀和满足题意的情况
        remain_to_first_index.insert(0, -1);
        let mut pre_sum = 0;
        for (i, num) in nums.into_iter().enumerate() {
            // 计算 nums[..=i] 的前缀和及模 k 的余数
            let i = i as _;
            pre_sum += num;
            let remain = pre_sum % k;
            if let Some(j) = remain_to_first_index.get(&remain) {
                ...
                remain_to_first_index.insert(remain, i);
            }
        }
