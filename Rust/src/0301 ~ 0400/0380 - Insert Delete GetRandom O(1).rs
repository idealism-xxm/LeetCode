...
//          4. int getRandom(): 随机返回集合中一个数。（调用时保证集合中至少存在一个数）
//              需要保证每个数字被返回的可能性都相等。
//
...
struct RandomizedSet {
    ...
    nums: Vec<i32>,
    // num_to_index[num] 表示 num 在 nums 中的下标，用于 O(1) 插入/删除一个数字
...
impl RandomizedSet {
    ...
    fn insert(&mut self, val: i32) -> bool {
        ...
        if self.num_to_index.contains_key(&val) {
            return false;
        }
        
        // 否则将 val 放入 nums 中，并记录其在 nums 中的下标
        self.num_to_index.insert(val, self.nums.len());
        self.nums.push(val);
        true
    }
    ...
    fn remove(&mut self, val: i32) -> bool {
        ...
        let index = self.num_to_index[&val];
        let last_index = self.nums.len() - 1;
        // 将最后一个数移动至 index 处，并更新其下标
        self.num_to_index.insert(self.nums[last_index], index);
        self.nums[index] = self.nums[last_index];

        // O(1) 移除最后一个数字（即 val ）
        self.num_to_index.remove(&val);
        self.nums.pop();
        true
    }
