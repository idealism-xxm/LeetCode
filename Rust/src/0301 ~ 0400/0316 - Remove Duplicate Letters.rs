...
//          2. 需要维护 is_in_stack ，存储全部 O(K) 个不同的字符
...
impl Solution {
    pub fn remove_duplicate_letters(s: String) -> String {
        ...
        // is_in_stack[ch] 表示 ch 是否在栈中
        let mut is_in_stack = HashMap::new();
        // 用栈 stack 收集当前的结果
        let mut stack: Vec<char> = Vec::new();
        // 带下标遍历 s 中的字符
        for (i, ch) in s.chars().enumerate() {
            // 如果当前字符 ch 不在栈中，则需要入栈
            if !is_in_stack.contains_key(&ch) {
                // 如果栈顶字符 top 比 ch 大，且 top 不是 s 中最后一个字符，
                // 那么那可以先出栈 top ，这样后续再放入时，字典序必定更小
                while !stack.is_empty() && stack.last().unwrap() > &ch && last_index[stack.last().unwrap()] > i {
                    // 栈顶字符 top 出栈，并标记为不在栈中
                    is_in_stack.remove(&stack.pop().unwrap());
                }

                // 将当前字符 ch 入栈
                stack.push(ch);
                // 标记当前字符 ch 为在栈中
                is_in_stack.insert(ch, true);
            }
        }
