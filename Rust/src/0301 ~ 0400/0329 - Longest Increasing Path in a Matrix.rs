...
impl Solution {
    pub fn longest_increasing_path(matrix: Vec<Vec<i32>>) -> i32 {
        ...
        let mut cells = matrix
            ...
            // 带上下标
            .enumerate()
            // 将 matrix[r][c] 转换成 (matrix[r][c], r, c)
            .flat_map(|(r, row)| row.iter().enumerate().map(move |(c, &v)| (v, r, c)))
            // 收集成 Vec<(i32, usize, usize)>
            .collect::<Vec<_>>();
        // 按照单元格的值从小到大排序
        cells.sort();
        ...
        for (v, r, c) in cells {
            for &(dr, dc) in DIRS.iter() {
                ...
                if 0 <= rr && rr < m as i32 &&
                     0 <= cc && cc < n as i32 &&
                     matrix[rr as usize][cc as usize] < v {
                      // 如果 (rr, cc) 的值比 (r, c) 的值小，
                      // 则 (r, c) 的前一个单元格可以是 (rr, cc) ，
                      // 即 dp[r][c] 可以由 dp[rr][cc] + 1 转移而来
                      dp[r][c] = dp[r][c].max(dp[rr as usize][cc as usize] + 1);
                 }
            }
            // 更新 ans
            ans = ans.max(dp[r][c]);
