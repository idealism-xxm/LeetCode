...
// 思路： Map
//
//      我们可以用一个名为 first_index 的 map 维护 s 中每种字符第一次出现的下标。
//
//      枚举 s 中的第 i 个字母 ch ：
//          1. ch 不在 first_index 中: 则 ch 是第一次出现，
//              记录其下标即可， first_index[ch] = i
//          2. ch 在 first_index 中: 则 ch 是第二次出现，
//              判断其是否满足题意。
//              如果 i - first_index[ch] - 1 != distance[ch - 'a'] ，
...
impl Solution {
    pub fn check_distances(s: String, distance: Vec<i32>) -> bool {
        // first_index 维护每种字母第一次出现的下标
        let mut first_index = HashMap::<u8, usize>::with_capacity(s.len() >> 1);
        // 遍历 s 中的第 i 个字母 ch
        for (i, &ch) in s.as_bytes().iter().enumerate() {
            if let Some(j) = first_index.get(&ch) {
                // ch 是第二次出现，如果其满足题意，则直接返回 false
                if i - j - 1 != distance[(ch - b'a') as usize] as usize {
                    return false;
                }
            } else {
                // ch 是第一次出现，记录其下标即可
                first_index.insert(ch, i);
            }
        }
