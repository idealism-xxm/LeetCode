...
struct TimeMap {
    key_to_values: HashMap<String, Vec<(i32, String)>>
}
...
impl TimeMap {
    ...
    fn new() -> Self {
        TimeMap { key_to_values: HashMap::new() }
    }
    ...
    fn get(&self, key: String, timestamp: i32) -> String {
        self.key_to_values
            ...
            .map_or_else(|| "".to_string(), |values| {
                // 存在就找到 values 中时间点第一个大于 timestamp 的值的下标
                let k = values.partition_point(|&(t, _)| t <= timestamp);
                if k == 0 {
                    // k == 0 表示不存在这样的值，则返回 ""
                    "".to_string()
                } else {
