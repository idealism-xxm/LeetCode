...
impl Solution {
    pub fn close_strings(word1: String, word2: String) -> bool {
        ...
        // 获取两个字符串的字符集和统计信息
        let (word1_ch_set, word1_ch_cnt_to_cnt) = Self::get_statistics(&word1);
        let (word2_ch_set, word2_ch_cnt_to_cnt) = Self::get_statistics(&word2);
        
        // 当两个字符串字符集和统计信息都相等时，可以相互转换
        return word1_ch_set == word2_ch_set && word1_ch_cnt_to_cnt == word2_ch_cnt_to_cnt
    }

    fn get_statistics(word: &String) -> (HashSet<u8>, HashMap<usize, usize>) {
        // ch_to_cnt[ch] 表示 word 中 ch 的出现次数
        ...
        // ch_set 维护 word 的字符集
        let ch_set = ch_to_cnt.keys().map(|&ch| ch).collect();

        // ch_cnt_to_cnt[ch_cnt] 表示 word 中出现 ch_cnt 次的字符的个数
        let mut ch_cnt_to_cnt = HashMap::new();
        for &ch_cnt in ch_to_cnt.values() {
            ch_cnt_to_cnt.entry(ch_cnt).or_insert(0).add_assign(1);
        }

        (ch_set, ch_cnt_to_cnt)
    }
}
