...
impl Solution {
    pub fn find_ball(grid: Vec<Vec<i32>>) -> Vec<i32> {
        ...
        // 枚举每个球，模拟其运动轨迹
        for c in 0..n {
            ans.push(Self::get_final_column(&grid, c));
        }
        ans
    }
    
    fn get_final_column(grid: &Vec<Vec<i32>>, mut c: usize) -> i32 {
        let (m, n) = (grid.len(), grid[0].len());
        // 模拟初始在 (0, c) 处的球的运动轨迹
        for r in 0..m {
            // 计算 (r, c) 处的球可能导向的位置
            let cc = (c as i32 + grid[r][c]) as usize;
            // 如果导向了箱子侧边 (cc < 0 || cc >= n) ，
            //    ( cc 类型为 usize ，所以会下溢，无需判断是否小于 0 ）
            // 或导向了 'V' 形图案 (grid[r][c] != grid[r][cc]) ，
            // 则球会被卡在此处，返回 -1
            if cc >= n || grid[r][c] != grid[r][cc] {
                return -1;
            }
