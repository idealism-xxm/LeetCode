...
//
//      只需要对 box_types 按照 box_types[i][1] 倒序排序，
//      然后按顺序选取前 truckSize 个箱子即可。
//
//
//      时间复杂度：O(nlogn)
//          1. 需要对 box_types 进行排序，时间复杂度为 O(nlogn)
...
impl Solution {
    pub fn maximum_units(mut box_types: Vec<Vec<i32>>, mut truck_size: i32) -> i32 {
        ...
        box_types.sort_by_key(|box_type| Reverse(box_type[1]));

        let mut ans = 0;
        for box_type in box_types {
            if truck_size <= box_type[0] {
                // 如果当前类型的箱子能用完选取次数，则选取 truck_size 个箱子后，跳出循环
