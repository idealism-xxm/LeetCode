...
impl Solution {
    pub fn longest_str_chain(mut words: Vec<String>) -> i32 {
        ...
        let mut dp = HashMap::with_capacity(words.len());
        // 初始化最长单词链的长度为 0
        let mut ans = 0;
        // 按照字符串长度升序排序
        ...
        for word in words {
            ...
            let mut max_len = 0;
            for i in 0..word.len() {
                let pre = String::with_capacity(word.len()) + &word[..i] + &word[i + 1..];
                max_len = max_len.max(*dp.get(&pre).unwrap_or(&0));
            }
            // 转移至以 word 为结尾的最长单词链的长度
            dp.insert(word, max_len + 1);
            // 更新最长单词链的长度
            ans = ans.max(max_len + 1);
        }
