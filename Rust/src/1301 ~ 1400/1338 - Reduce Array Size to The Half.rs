...
impl Solution {
    pub fn min_set_size(arr: Vec<i32>) -> i32 {
        ...
        // 转换成一个数组，方便排序
        let mut cnt_with_nums = num_to_cnt
            .iter()
            // 对 cnt 包一层 Reverse ，方便对 cnt 降序排序
            .map(|(&num, &cnt)| (Reverse(cnt), num))
            .collect::<Vec<_>>();
        // 先按 cnt 降序排序，再按 num 升序排序
        cnt_with_nums.sort_unstable();

        // ans 表示集合的大小
        let mut ans = 0;
        // total 表示已经移除的数字的个数
        let mut total = 0;
        for (Reverse(cnt), num) in cnt_with_nums {
            // 计入当前数字及其出现次数
            ans += 1;
            total += cnt;
