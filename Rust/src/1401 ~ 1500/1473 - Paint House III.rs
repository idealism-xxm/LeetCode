...
// 思路： DP
//
//      设 dp[i][j][k] 表示前 i 个房子中，共有 k 个街区，
//      且第 i 个房子的颜色为 j 时的最小花费。
//
...
impl Solution {
    pub fn min_cost(houses: Vec<i32>, cost: Vec<Vec<i32>>, m: i32, n: i32, target: i32) -> i32 {
        ...
        let mut dp = vec![vec![vec![MAX; n]; target + 1]; m];
        if houses[0] == 0 {
        ...
        for i in 1..m {
            ...
            for j in 1..=target {
                ...
                for k in 0..n {
                    ...
                    if houses[i] != 0 && houses[i] as usize != k + 1 {
                        continue;
                    }
                    // 计算把第 i 个房子染成颜色 k 需要的花费
                    let paint_cost = if houses[i] == 0 {
                    ...
                    for l in 0..n {
                        if k == l {
                            ...
                            dp[i][j][k] = dp[i][j][k].min(dp[i - 1][j][l] + paint_cost);
                        } else {
                            // 如果颜色不同，会产生新的街区，
                            // 则依赖的状态为前 i - 1 个房子的街区数为 j - 1
                            dp[i][j][k] = dp[i][j][k].min(dp[i - 1][j - 1][l] + paint_cost);
                        }
                    }
                }
            }
        ...
        let ans = *dp[m - 1][target].iter().min().unwrap();
        if ans == MAX {
