...
// 输出： 9
// 解释： 面积最大的蛋糕（标记为 1 ）的高为 3 ，宽为 3 ，面积为 3 * 3 = 9
//       111|0
//       111|0
//       111|0
//       -----
...
//
//      那么可以贪心地分别求横切的最大间隔 max_h 和竖切的最大间隔 max_w ，
//      那么面积最大的蛋糕大小为 max_h * max_w 。
//
...
impl Solution {
    pub fn max_area(h: i32, w: i32, mut horizontal_cuts: Vec<i32>, mut vertical_cuts: Vec<i32>) -> i32 { 
        // 获取最大矩形的高和宽，并计算面积
        let (max_h, max_w) = (Self::get_max_interval(horizontal_cuts, h), Self::get_max_interval(vertical_cuts, w));
        ((max_h * max_w) % 1000000007) as i32
    }

    fn get_max_interval(mut vals: Vec<i32>, max_val: i32) -> i64 {
        // 按升序排序，然后计算相邻值的最大间隔
        vals.sort();
        ...
        let mut pre_val = 0;
        for val in vals {
            max_interval = max_interval.max(val - pre_val);
            pre_val = val;
        }

        // 计算最后一个间隔值，并返回最大值
        max_interval.max(max_val - pre_val) as i64
    }
}
