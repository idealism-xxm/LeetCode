...
//
//      同时我们用 num_to_cnt 维护滑动窗口 [l, r] 内每个数字的出现次数。
...
//      那么还需要从 num_to_cnt 移除 nums[l] ，
//      以保证 num_to_cnt 中维护的都是滑动窗口内的数字的出现次数。
//
//      此时，如果 num_to_cnt 的大小为 k ，
...
//      空间复杂度：O(k)
//          1. 需要维护 num_to_cnt 中全部 O(k) 个数字的出现次数
...
impl Solution {
    pub fn maximum_subarray_sum(nums: Vec<i32>, k: i32) -> i64 {
        ...
        let mut sum = 0;
        // num_to_cnt 表示滑动窗口 [l, r] 内每个数字的出现次数
        let mut num_to_cnt = HashMap::new();
        // 不断右移滑动窗口右边界 r
        for r in 0..nums.len() {
            // 将 nums[r] 纳入滑动窗口中考虑
            num_to_cnt.entry(nums[r]).or_insert(0).add_assign(1);
            sum += nums[r] as i64;
            // 如果 r >= k ，则需要将 nums[r - k] 从滑动窗口中移除，
            // 以保持滑动窗口长度为 k
            if r >= k {
                num_to_cnt.entry(nums[r - k]).or_insert(0).sub_assign(1);
                sum -= nums[r - k] as i64;
                // 如果移除后， nums[r - k] 的出现次数为 0 ，
                // 还需要从 num_to_cnt 移除 nums[l] ，
                // 以保证 num_to_cnt 中维护的都是滑动窗口内的数字的出现次数
                if num_to_cnt[&nums[r - k]] == 0 {
                    num_to_cnt.remove(&nums[r - k]);
                }
            }

            // 如果 num_to_cnt 的大小为 k ，
            // 那么说明滑动窗口 [l, r] 内的全部 k 个数字各不相同，
            // 更新 ans 的最大值
            if num_to_cnt.len() == k {
                ans = ans.max(sum);
