...
impl Solution {
    pub fn most_popular_creator(creators: Vec<String>, ids: Vec<String>, views: Vec<i32>) -> Vec<Vec<String>> {
        ...
        let mut creator_to_info = HashMap::new();
        // 创作者所有电影的观看数之和的最大值
        let mut max_view = 0;
        for (i, creator) in creators.into_iter().enumerate() {
            // 获取创作者 creator 的相关信息
            let (index, view) = creator_to_info.entry(creator).or_insert((i, 0));
            // 将第 i 部电影的观看数计入 view
            *view += views[i] as i64;
            // 如果第 i 部电影的观看数更多，或者观看次数相等但字典序更小，
            // 则贪心地选择第 i 部电影
            if views[i] > views[*index] || (views[i] == views[*index] && ids[i] < ids[*index]) {
                *index = i
            }
        ...
        creator_to_info
            .into_iter()
            // 仅过滤出观看数之和最大的
            .filter(|&(_, (_, view))| view == max_view)
            // 转换成需要的形式
            .map(|(creator, (index, _))| vec![creator, ids[index].clone()])
            // 收集成列表后返回
            .collect()
    }
}
