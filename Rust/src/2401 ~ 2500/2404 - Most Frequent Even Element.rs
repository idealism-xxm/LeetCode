...
impl Solution {
    pub fn most_frequent_even(nums: Vec<i32>) -> i32 {
        ...
        let mut even_to_cnt = HashMap::new();
        for num in nums {
            if num & 1 == 0 {
                even_to_cnt.entry(num).or_insert(0).add_assign(1);
            }
        }

        // ans 表示出现次数最多的偶数，初始化为 -1 ，表示暂无偶数。
        // max_cnt 表示 ans 的出现次数，初始化为 0 ，表示暂无偶数。
        let (mut ans, mut max_cnt) = (-1, 0);
        for (&even, &cnt) in even_to_cnt.iter() {
            if max_cnt < cnt {
                // 如果当前偶数 even 的出现次数更多，那么直接更新 ans 和 max_cnt
                ans = even;
                max_cnt = cnt;
            } else if max_cnt == cnt && even < ans {
