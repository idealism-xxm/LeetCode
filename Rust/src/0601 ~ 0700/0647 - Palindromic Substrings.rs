...
//      就能在 O(1) 内判断 s[l-1..=r+1] 是否是回文子串。
//
//      我们可以枚举回文子串的中心点，由于字符串的长度可能是奇数和偶数，
//      而每次往外扩展时，都会新增两个字符，子串的奇偶性不会改变。
//
//      所以我们需要分别枚举中心点为一个字符 s[i..=i] 和两个字符 s[i..=i+1] 的情况，
//      找到这两种情况下的回文子串的数量。
//
//      为了方便处理，我们定义一个函数 count(s, l, r) ，
//      用来统计以 s[l..=r] 为中心的回文子串数量。
//
//      这样只需要遍历一遍字符串的下标 i ，
...
impl Solution {
    ...
    // 统计以 s[l..=r] 为中心的回文子串数量
    fn count(s: &[u8], l: usize, mut r: usize) -> i32 {
        // 为了避免 usize 下溢到最大值而造成判断错误
        let mut l = l as i32;
        // ans 表示以 s[l..=r] 为中心的回文子串数量
        let mut ans = 0;
        // 如果 l 和 r 合法，且 s[l] == s[r]，
        // 则 s[l..=r] 是一个回文子串，令 ans += 1
...
//
//      设 dp[l][r] 表示 s[l..=r] 是否为回文子串，初始化为 false。
//
//      所有长度为 1 的子串为回文子串，
//      即 dp[r][r] = true; ans += 1;
//
//      如果相邻的两个字符 s[r-1] 和 s[r] 相等，
//      则 s[r-1..=r] 是长度为 2 的回文子串，
//      即 dp[r-1][r] = true; ans += 1;
//
//      状态转移：枚举长度为 length 的子串，再枚举子串的结束下标 r ，
//      可计算出子串的开始下标 l = r - length + 1 。
//
//      如果此时 s[l] == s[r] 且 s[l+1..=r-1] 是回文子串，
//      则 s[l..=r] 是长度为 length 的回文子串，
//      即 dp[l][r] = true; ans += 1;
//
//
//      时间复杂度：O(n ^ 2)
...
impl Solution {
    pub fn count_substrings(s: String) -> i32 {
        ...
        // ans 表示 s 中所有回文子串的数量
        let mut ans = 0;
        // dp[l][r] 表示 s[l..=r] 是否是回文子串，初始化为 false
        ...
        for r in 1..n {
            ...
            dp[r][r] = true;
            ans += 1;
            // 如果 s[r - 1] == s[r] ，则 s[r-1..=r] 是长度为 2 的回文子串
            if s[r - 1] == s[r] {
                dp[r - 1][r] = true;
                ans += 1;
            }
