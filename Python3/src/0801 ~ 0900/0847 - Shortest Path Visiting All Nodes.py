...
#
#       再判断这种状态是否遍历过，
#       如果没有遍历过，即 dist[nxt][next_state] == i32::MAX 时，可以继续处理。
#
#       如果 next_state 已遍历完所有结点，则直接返回 dist[cur][state] + 1 。
#       否则，更新 dist[nxt][next_state] 的最短路径长度为 dist[cur][state] + 1 ，
...
class Solution:
    def shortestPathLength(self, graph: List[List[int]]) -> int:
        ...
        # 已遍历过的结点状态为 j 时的最短路径长度，
        # 初始化都是 0x3fffffff
        dist = [[0x3fffffff] * (mx + 1) for _ in range(len(graph))]
        # q 用于 BFS ，存储后续可以遍历的结点等信息
        q = deque([])
        # 初始以每个结点为开始结点
        for i in range(len(graph)):
            # 初始在结点 i ，已遍历过的结点状态为 1 << j 时，
            # 最短路径长度为 0 ，可以放入 BFS 继续处理
            dist[i][1 << i] = 0
            q.append((i, 1 << i))
        ...
        while True:
            ...
            for nxt in graph[cur]:
                # 计算此时移动到 nxt 的对应状态 next_state
                next_state = state | (1 << nxt)
                # 如果这种状态目前没有遍历过，则可以进行遍历
                if dist[nxt][next_state] == 0x3fffffff:
                    ...
                    dist[nxt][next_state] = dist[cur][state] + 1
                    q.append((nxt, next_state))
