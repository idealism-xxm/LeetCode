...
class Solution:
    def checkSubarraySum(self, nums: List[int], k: int) -> bool:
        ...
        remain_to_first_index: Dict[int, int] = {}
        # 初始前缀和 0 % k 的下标为 -1 ，方便处理整个前缀和满足题意的情况
        remain_to_first_index[0] = -1
        pre_sum: int = 0
        for i, num in enumerate(nums):
            # 计算 nums[..=i] 的前缀和及模 k 的余数
            pre_sum += num
            remain: int = pre_sum % k
            j: Optional[int] = remain_to_first_index.get(remain)
            if j is None:
                # 如果 remain 是第一次出现，则记录其下标
                remain_to_first_index[remain] = i
            elif i - j > 1:
