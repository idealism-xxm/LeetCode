...
class Solution:
    def findPaths(self, m: int, n: int, maxMove: int, startRow: int, startColumn: int) -> int:
        ...
        # 初始时，移动 0 次，只有 (startRow, startColumn) 位置可能有球，方案数为 1
        dp[startRow][startColumn] = 1
        ans: int = 0
        # 【注意】这里只统计转移到 maxMove - 1 次移动，因为需要额外的一次移动移出网格
        for _ in range(maxMove):
            # 定义第 i + 1 次时网格的状态，初始化都为 0
            next_dp: List[List[int]] = [[0] * n for _ in range(m)]
            for r in range(m):
                for c in range(n):
                    ...
                    for dr, dc in DIRS:
                        ...
                        if 0 <= rr < m and 0 <= cc < n:
                            next_dp[rr][cc] = (next_dp[rr][cc] + dp[r][c]) % MOD
