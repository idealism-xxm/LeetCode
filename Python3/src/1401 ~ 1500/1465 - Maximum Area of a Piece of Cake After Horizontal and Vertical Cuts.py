...
class Solution:
    def maxArea(self, h: int, w: int, horizontalCuts: List[int], verticalCuts: List[int]) -> int:
        # 获取最大矩形的高和宽，并计算面积
        max_h, max_w = Solution.get_max_interval(horizontalCuts, h), Solution.get_max_interval(verticalCuts, w)
        return (max_h * max_w) % 1000000007

    @staticmethod
    def get_max_interval(vals: List[int], max_val: int) -> int:
        # 按升序排序，然后计算相邻值的最大间隔
        vals.sort()
        max_interval: int = 0
        # 起始值为 0 ，方便计算第一个间隔
        pre_val: int = 0
        for val in vals:
            max_interval = max(max_interval, val - pre_val)
            pre_val = val

        # 计算最后一个间隔值，并返回最大值
        return max(max_interval, max_val - pre_val)
