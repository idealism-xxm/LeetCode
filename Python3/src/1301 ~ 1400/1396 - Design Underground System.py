...
class UndergroundSystem:
    ...
    def __init__(self):
        ...
        self.id_to_start_time: Dict[int, (str, int)] = {}
        # 维护出入站为 (start_station, end_station) 的所有用户的个数与总耗时
        self.travel_to_info: Dict[(str, str), List[int]] = defaultdict(lambda: [0, 0])

    def checkIn(self, id: int, stationName: str, t: int) -> None:
        # 将用户 id 的入站信息放入 id_to_start_time 中
        self.id_to_start_time[id] = [stationName, t]

    def checkOut(self, id: int, stationName: str, t: int) -> None:
        # 获取用户 id 的入站信息
        start_station, start_time = self.id_to_start_time.pop(id)
        # 获取出入站为 (start_station, end_station) 的所有用户的个数与总耗时信息
        info: List[int] = self.travel_to_info[start_station, stationName]
        # 加上用户 id 的出入站次数
        info[0] += 1
        # 加上用户 id 的出入站耗时
        info[1] += t - start_time
    ...
    def getAverageTime(self, startStation: str, endStation: str) -> float:
        # 获取出入站为 (start_station, end_station) 的所有用户的个数与总耗时信息
        cnt, total_time = self.travel_to_info[startStation, endStation]
        # 计算平均耗时
