...
class Solution:
    def maxAncestorDiff(self, root: Optional[TreeNode]) -> int:
        ...
        return Solution.dfs(root, root.val, root.val)

    @staticmethod
    def dfs(root: Optional[TreeNode], low: int, high: int) -> int:
        # 如果 root 为空，则直接返回 0
        if not root:
            return 0

        # 如果 root 不为空，则继续计算结点 root 相关的 diff 的最大值
        ans: int = max(abs(low - root.val), abs(high - root.val))
        # 更新祖先结点值的最小值和最大值
        low = min(low, root.val)
        high = max(high, root.val)
        # 递归处理左右子树，并更新 ans 的最大值
        ans = max(ans, Solution.dfs(root.left, low, high))
        ans = max(ans, Solution.dfs(root.right, low, high))
