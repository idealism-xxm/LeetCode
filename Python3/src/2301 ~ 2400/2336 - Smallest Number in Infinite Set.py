...
class SmallestInfiniteSet:
    ...
    def __init__(self):
        ...
        self.deleted_nums = set()
        # min_num 维护无限集合中最小的数字，初始未删除任何数字，最小的数字就是 1
        self.min_num = 1

    def popSmallest(self) -> int:
        ans: int = self.min_num
        # 将 min_num 删除，放入到 deleted_nums 中
        self.deleted_nums.add(self.min_num)
        # 不断将 min_num 加 1 ，直至 min_num 不在 deleted_nums 中
        while self.min_num in self.deleted_nums:
            self.min_num += 1
