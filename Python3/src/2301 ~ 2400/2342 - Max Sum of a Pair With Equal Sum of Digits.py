...
class Solution:
    def maximumSum(self, nums: List[int]) -> int:
        digit_sum_to_largest_two_nums: Dict[List[int]] = defaultdict(lambda: [0, 0])
        for num in nums:
            # 计算 num 的数位和
            digit_sum: int = 0
            remain: int = num
        ...
        for largest, next_lartext in digit_sum_to_largest_two_nums.values():
            # 只处理有最大两个数情况
            if next_lartext > 0:
                # 计算数字和并更新 ans 最大值
                ans = max(ans, largest + next_lartext)
