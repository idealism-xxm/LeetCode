...
class Solution:
    def isInterleave(self, s1: str, s2: str, s3: str) -> bool:
        ...
        m, n, s3_len = len(s1), len(s2), len(s3)
        if m + n != s3_len:
            return False

        # dp[j] 表示 s1[..i] 与 s2[..j] 是否能交错组成 s3[..i + j]
        dp: List[int] = [False] * (n + 1)
        for i in range(m + 1):
            for j in range(n + 1):
                ...
                is_last_ch_from_s1, is_last_ch_from_s2 = False, False
                ...
                if j > 0 and dp[j - 1] and s2[j - 1] == s3[i + j - 1]:
                    is_last_ch_from_s2 = True

                # 只要能从 dp[i - 1][j] 和 dp[i][j - 1] 任意一个状态转移而来，
                # dp[i][j] 就为 true
                dp[j] = is_last_ch_from_s1 or is_last_ch_from_s2
