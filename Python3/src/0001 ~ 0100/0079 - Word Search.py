...
class Solution:
    def exist(self, board: List[List[str]], word: str) -> bool:
        # 先进行剪枝和优化
        word: Optional[str] = Solution.prune(board, word)
        ...
        for r in range(len(board)):
            for c in range(len(board[0])):
                if Solution.dfs(board, word, 0, r, c):
                    return True
    ...
    @staticmethod
    def prune(board: List[List[str]], word: str) -> Optional[str]:
        ...
        cell_to_cnt: Dict[str, int] = Counter(ch for row in board for ch in row)
        ch_to_cnt: Dict[str, int] = Counter(word)
        # 如果 word 中某种字符的数量在 board 中不够，则直接返回空
        for ch, cnt in ch_to_cnt.items():
            if cell_to_cnt[ch] < cnt:
                return None
        
        # 选择搜索空间更小的一端
        l, r = 0, len(word) - 1
        while l < r:
            lcnt, rcnt = ch_to_cnt[word[l]], ch_to_cnt[word[r]]
            if lcnt == rcnt:
    ...
    @staticmethod
    def dfs(board: List[List[str]], word: str, cur: int, r: int, c: int) -> bool:
        ...
        for dr, dc in DIRS:
            ...
            if (
                Solution.is_valid(m, n, rr, cc) and
                Solution.dfs(board, word, cur + 1, rr, cc)
            ):
                return True
    ...
    @staticmethod
    def is_valid(m: int, n: int, r: int, c: int) -> bool:
