...
class Solution:
    ...
    @staticmethod
    def dfs(root: Optional[TreeNode], row: int, col: int, result: List[Tuple[int, int, int]]):
        # 如果是空结点，则直接返回
        if not root:
            return

        # 把当前结点的坐标和值放入 ans 中
        result.append((col, row, root.val));
        # 递归处理左右子树
        Solution.dfs(root.left, row + 1, col - 1, result)
        Solution.dfs(root.right, row + 1, col + 1, result)
