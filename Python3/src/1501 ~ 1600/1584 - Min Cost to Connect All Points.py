...
class Solution:
    def minCostConnectPoints(self, points: List[List[int]]) -> int:
        ...
        while mst_size < n:
            ...
            nxt: int = 0
            # 表示 nxt 距离当前最小生成树的距离
            nxt_dist: int = 0x3f3f3f3f
            # 找到当前最小生成树外，距离最小生成树最近的点
            for i in range(n):
                # 如果点 i 不在最小生成树中，
                # 且距离当前最小生成树的距离更小
                if not in_mst[i] and dist[i] < nxt_dist:
                    # 更新 i 为下一个可放入的点
                    nxt = i
                    # 更新 i 到当前最小生成树的距离
                    nxt_dist = dist[i]
            ...
            for i in range(n):
                # 计算 i 到 nxt 的距离
                i_dist: int = abs(points[nxt][0] - points[i][0]) + \
                              abs(points[nxt][1] - points[i][1])
                # 如果 i 不在最小生成树中，且到 nxt 的距离更小
                if not in_mst[i] and dist[i] > i_dist:
