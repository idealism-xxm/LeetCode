...
class Solution:
    def lowestCommonAncestor(self, root: 'TreeNode', p: 'TreeNode', q: 'TreeNode') -> 'TreeNode':
        return Solution.dfs(root, p, q)[0]

    @staticmethod
    def dfs(root: Optional['TreeNode'], p: 'TreeNode', q: 'TreeNode') -> Tuple['TreeNode', int]:
        # root 为空，则直接返回 None, 0
        if not root:
            return None, 0

        # 递归计算左子树结果 left_node, left_cnt
        left_node, left_cnt = Solution.dfs(root.left, p, q)
        # 若 left_cnt == 2 ，则 left_node 就是 LCA ，直接返回即可
        if left_cnt == 2:
            return left_node, 2

        # 递归计算右子树结果 right_node, right_cnt
        right_node, right_cnt = Solution.dfs(root.right, p, q)
        # 若 right_cnt == 2 ，则 right_node 就是 LCA
        if right_cnt == 2:
            return right_node, 2

        # 计算当前子树含有的 p 和 q 的结点数
        cur_cnt: int = left_cnt + right_cnt
