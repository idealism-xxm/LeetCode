...
class Solution:
    ...
    @staticmethod
    def dfs(root: Optional[TreeNode], level: int, index: int, level_left_index: List[int]) -> int:
        # 如果 root 非空，则可以继续递归处理
        if root is not None:
            # 如果当前层还没有最左侧结点的索引，
            # 那么当前结点就是该层的最左侧结点，放入到 level_left_index 中
            if level >= len(level_left_index):
                level_left_index.append(index)

            # 先递归处理，获取左右子树中的最大宽度，并取最大值
            max_width: int = max(
                Solution.dfs(root.left, level + 1, index * 2, level_left_index),
                Solution.dfs(root.right, level + 1, index * 2 + 1, level_left_index),
            )

            # 然后再计算以当前结点为最右侧结点时的宽度，取最大值并返回
            return max(max_width, index - level_left_index[level] + 1)
