...
//
//      针对每个单词都枚举其中的字母 ch ，
//      每次都贪心地在 s[idx..] 中找到 ch 第一次出现的下标 nextIdx 。
//
//      因为如果找第二次或者后续出现的下标， s 中剩余可用于匹配的字符会变少，答案不会更优。
//
//      针对找到的下标 nextIdx ，有以下两种情况：
//          1. nextIdx == len(s): 即 ch 在 s[idx..] 中不存在，则 word 不是 s 的子序列，
//              直接处理下一个单词
//          2. nextIdx < len(s): 更新 idx = nextIdx + 1 ，需要继续循环处理，
//              即需要判断单词剩余的字符是否为 s[nextIdx + 1..] 的子序列
//
...
func numMatchingSubseq(s string, words []string) int {
    ...
    for _, word := range words {
        ...
        isSubsequence := true
        for _, ch := range word {
            // 找到当前字母在 s[idx..] 中第一次出现的下标 nextIdx
            nextIdx := dp[idx][ch - 'a']
            ...
            // 如果存在，则下一次需要判断剩余子串是否为 s[nextIdx+1..] 的子序列
            idx = nextIdx + 1
        }
        // 如果是子序列，则计入到 ans 中
        if isSubsequence {
