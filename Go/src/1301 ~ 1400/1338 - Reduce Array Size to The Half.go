...
//
//      所以我们可以先统计每个数字的出现次数到 numToCnt 中，然后按照出现次数降序排序。
...
//      空间复杂度：O(n)
//          1. 需要维护 numToCnt 中全部不同数字的出现次数，最差情况下有 O(n) 个不同数字
//          2. 需要用一个数组维护全部不同的数字，并用于排序，最差情况下有 O(n) 个不同数字


func minSetSize(arr []int) int {
    n := len(arr)
    // numToCnt[num] 表示 num 的出现次数
    numToCnt := make(map[int]int)
    // 遍历 arr 中全部数字，统计每个数字的出现次数
    for _, num := range arr {
        numToCnt[num] += 1
    }

    // 转换成一个数组，方便排序
    nums := make([]int, 0, len(numToCnt))
    for num := range numToCnt {
        nums = append(nums, num)
    }
    // 先按 cnt 降序排序，再按 num 升序排序
    sort.Slice(nums, func(i, j int) bool {
        if numToCnt[nums[i]] != numToCnt[nums[j]] {
            return numToCnt[nums[i]] > numToCnt[nums[j]]
        }
