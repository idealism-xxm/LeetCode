...
//
//      所以我们可以用一个名为 diffToNums 的 map 收集所有对角线上的数字，
//      然后将每个对角线上的数字按降序排序。
//
...
func diagonalSort(mat [][]int) [][]int {
    ...
    // diffToNums[diff] 维护对角线 i - j 上的所有数字
    diffToNums := make(map[int][]int)
    // 收集每个对角线的数字
    for r := 0; r < m; r++ {
        for c := 0; c < n; c++ {
            diffToNums[r - c] = append(diffToNums[r - c], mat[r][c])
        }
    }
    // 对每个对角线上的数字按降序排序，
    ...
    for r := 0; r < m; r++ {
        for c := 0; c < n; c++ {
            nums := diffToNums[r - c]
            mat[r][c] = nums[len(nums) - 1]
            diffToNums[r - c] = nums[:len(nums) - 1]
        }
    }
