...
//
//      如果我们使用指针 curA 先遍历链表 a ，再遍历链表 b ；
//      然后用指针 curB 先遍历链表 b ，再遍历链表 a 。
//
//      那么 curA 和 curB 实际上就是将链表 b 和链表 a 进行了尾部对齐，
//      可以直接同时遍历。
//
//      我们分析一下不同的情况：
//          1. len(a) == len(b): 那么链表 a 和链表 b 本身就是对齐的，
//              在 curA 和 curB 第一次遍历 a 和 b 时，
//              就会走到相同结点（相交结点或空结点）。
//          2. a 和 b 不相交: 那么 curA 和 curB 在第二次遍历完 a 和 b 时，
//              就会走到空结点。
//          3. len(a) != len(b) && a 和 b 相交: 
//              那么 curA 和 curB 在均开始第二次遍历 a 和 b 时，
//              curA 和 curB 距离链表 b 和 a 尾部的长度相同，
