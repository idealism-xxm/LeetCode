...
//
//      很容易就会想到使用 DP 进行处理，
//      设 dp[i][j] 表示恰好用 i 步从 startPos 走到 j 的方案数。
//
//      初始化：初始从 startPos 恰好走 0 步到 startPos 的方案数为 1 。
//          即 dp[i][j] = 0; dp[i][startPos] = 1;
...
func numberOfWays(startPos int, endPos int, k int) int {
    // dp[pos] 表示从 startPos 恰好走 i 步到 pos 的方案数
    dp := make(map[int]int)
    // 初始从 startPos 恰好走 0 步到 startPos 的方案数为 1
    dp[startPos] = 1
    for i := 0; i < k; i++ {
        // 初始化第 i + 1 步的状态
    ...
    // 此时 dp[endPos] 就是从 startPos 恰好走 k 步到 endPos 的方案数
    return dp[endPos]
}
