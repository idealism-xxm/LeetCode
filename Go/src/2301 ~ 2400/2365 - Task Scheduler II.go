...
//
//      所以我们可以维护一个名为 minStartDay 第 map ，
//      minStartDay[y] 表示类型 y 的任务最小可以开始的天数。
//
...
//          2. 至少要在第 minStartDay[task] 天处理任务 task
//
//      取两者的最大值作为真正执行的天数，并更新 now 。
//
//      那么下一次任务 task 至少要在第 now + space + 1 天进行处理，
//      更新 minStartDay[task] = now + space + 1 即可。
//
...
//      空间复杂度：O(n)
//          1. 需要维护 minStartDay 中全部不同任务的最小可以开始天数，
//              最差情况下有 O(n) 个不同任务


func taskSchedulerII(tasks []int, space int) int64 {
    // minStartDay[y] 表示类型 y 的任务最小可以开始的天数
    minStartDay := make(map[int]int)
    ...
    for _, task := range tasks {
        ...
        //  2. 至少要在第 minStartDay[task] 天处理任务 task
        // 取两者的最大值作为真正执行的天数，并更新 now
        now = max(now + 1, minStartDay[task])
        // 则下一次任务 task 至少要在第 now + space + 1 天进行处理
        minStartDay[task] = now + space + 1
    }
