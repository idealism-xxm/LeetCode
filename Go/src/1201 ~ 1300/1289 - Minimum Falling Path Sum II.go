...
//
//      为此我们可以维护另一个数组 minDp ，其中 minDp[i] 表示 dp[i] 中最小的两个值，
//      其中 minDp[i][0] <= minDp[i][1] 。
//
//      这样在状态转移时，就能根据 minDp[i][0] 与 dp[i - 1][j] 是否相等，
//      选择 minDp[i - 1] 二者之一进行转移即可。    
//
//
...
func minFallingPathSum(grid [][]int) int {
    ...
    copy(dp[0], grid[0])
    // minDp[i] 表示 dp[i] 中最小的两个值，其中 minDp[i][0] <= minDp[i][1]
    minDp := make([][2]int, n)
    minDp[0][0], minDp[0][1] = math.MaxInt32, math.MaxInt32
    // 初始化 minDp[0]
    for j := range dp[0] {
        if dp[0][j] <= minDp[0][0] {
    ...
    for i := 1; i < n; i++ {
        minDp[i][0], minDp[i][1] = math.MaxInt32, math.MaxInt32
        for j := 0; j < n; j++ {
            if dp[i - 1][j] == minDp[i - 1][0] {
                // 如果 dp[i - 1][j] 是最小值，则选择次小值进行状态转移
                dp[i][j] = grid[i][j] + minDp[i - 1][1]
            } else {
                // 如果 dp[i - 1][j] 不是最小值，则选择最小值进行状态转移
                dp[i][j] = grid[i][j] + minDp[i - 1][0]
            }
