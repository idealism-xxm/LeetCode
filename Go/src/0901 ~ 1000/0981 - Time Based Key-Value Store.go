...
type TimeMap struct {
    keyToValues map[string][]*Value
}


func Constructor() TimeMap {
    return TimeMap {
        keyToValues: make(map[string][]*Value),
    }
}
...
func (this *TimeMap) Set(key string, value string, timestamp int) {
    ...
    this.keyToValues[key] = append(this.keyToValues[key], &Value{ timestamp, value })
}


func (this *TimeMap) Get(key string, timestamp int) string {
    // 获取 key 对应的 value 列表
    values := this.keyToValues[key]
    // 找到 values 中时间点第一个大于 timestamp 的值的下标
    k := sort.Search(len(values), func (i int) bool { return values[i].timestamp > timestamp });
    // k == 0 表示不存在这样的值，则返回 ""
    if k == 0 {
        return ""
    }
    // 此时 values[k - 1] 的值是 timestamp 及之前最后一次存储的
    return values[k - 1].value
}
