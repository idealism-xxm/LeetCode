...
//              放入 0 ，用于收集这一层括号内的分数和
//          2. 如果是右括号 ')' ，
//              则栈顶分数是当前要闭合的括号内的分数 innerScore ，
//              有两种可能：
//              (1) 形如 (): 则表明是第一种情况，此时 innerScore 必定为 0
//              (2) 形如 (A): 则表明是第三种情况，此时 innerScore 必定不为 0
//
//              综合这两种可能，那么此时要闭合的括号的分数为：
//              curScore = max(innerScore * 2, 1)
//
//              然后我们还要计算它所在层的分数和，即次栈顶就是当前层的分数和，
//              我们把 curScore 的分数加到次栈顶即可。
//
...
func scoreOfParentheses(s string) int {
    ...
    for _, ch := range s {
        if ch == '(' {
            ...
            top--
            // 综上，当前要闭合的括号的分数为： max(innerScore * 2, 1)
            curScore := 1
            if innerScore != 0 {
                curScore = innerScore << 1
            }
            // 获取当前要闭合的括号所在层的分数和
            scoreSum := stack[top]
            // 此时处理形如 AB 的第二种情况，
            // 即当前一层括号内的分数和为： scoreSum + curScore ，
            // 放入 stack 中，用于后续处理
            stack[top] = scoreSum + curScore
        }
    }
...
//      题目说明分数计算共有三种情况：
//          1. 第一种情况是 () ，会产生新分数贡献 1 。
//          2. 第二种情况是 AB ，将已有的分数执行加法，
//              不影响每一个 () 产生的贡献。
//          3. 第三种情况是 (A)，对已有的分数执行乘法，
//              即每增加一层， () 产生的分数贡献都会乘 2 。
//
//      这样我们只需要统计每个 () 的贡献即可，
//      如果一个 () 所在的深度为 depth ，
//      那么它对结果分数的贡献为 2 ^ depth 。
//
//      我们维护一个分数和 score 和当前括号的深度 depth ，
//      然后遍历第 i 个括号字符 ch ：
//          1. ch == '('，则 depth 加 1 ，
//          2. ch == ')'，则 depth 减 1 ，
//              如果前一个字符 s[i - 1] == '(' ，
//              那么说明当前闭合的是最内层的括号，即第一种情况，
//              则其对分数和的贡献为 2 ^ depth ，
//              统计贡献即可 score += 2 ^ depth 。
//
//
//		时间复杂度： O(n)
//          1. 需要遍历 s 中的全部 O(n) 个字符
