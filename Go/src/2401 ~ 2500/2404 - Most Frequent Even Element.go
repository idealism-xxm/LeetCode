...
// 思路： Map
//
//      用一个名为 evenToCnt 的 map 统计每个偶数出现的次数。
//
//      然后遍历 evenToCnt 中的每个数字 num ，找到其中出现次数最多偶数 ans 即可。
//
...
//          2. 需要遍历 evenToCnt 中全部不同的偶数，最差情况下有 O(n) 个
//      空间复杂度：O(n)
...
func mostFrequentEven(nums []int) int {
    ...
    // maxCnt 表示 ans 的出现次数，初始化为 0 ，表示暂无偶数。
    ans, maxCnt := -1, 0
    for even, cnt := range evenToCnt {
        if maxCnt < cnt {
            // 如果当前偶数 even 的出现次数更多，那么直接更新 ans 和 maxCnt
            ans = even
            maxCnt = cnt
        } else if maxCnt == cnt && even < ans {
